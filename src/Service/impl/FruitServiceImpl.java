...
public class FruitServiceImpl implements FruitService {
    ...
    public PageBean<Fruit> findFruitByPage(String _currentPage, String _rows, String condition) {
        ...
        //1.创建空的PageBean对象
        PageBean<Fruit> pb = new PageBean<Fruit>();
        //2.设置参数
        ...
        List<Fruit> list = dao.findFruitByPage(start, rows, condition);
        for (Fruit fruit : list) {
            try {
                String imgss[] = inimg.get_imgss(fruit.getImgs());
                fruit.setImgss(imgss);
    ...
    public int pull_offSelectedFruit(String[] ids) {
        ...
        if (ids != null && ids.length > 0) {
            ...
            for (String id : ids) {
                //2.调用dao删除
                a = dao.pull_offFruit(Integer.parseInt(id), now_time.format(new Date()));
            }
        }
        return a;
    }
    ...
    public int pull_offFruit(String id) {
        SimpleDateFormat now_time = new SimpleDateFormat("yyyy-MM-dd");
        return dao.pull_offFruit(Integer.parseInt(id), now_time.format(new Date()));
    }
    ...
    public int UpdateFruit(Fruit fru ,String []del_imgs) {
        try {
            String Title = new String(fru.getTitle().getBytes("ISO-8859-1"), "UTF-8");
            String Describes = new String(fru.getDescribes().getBytes("ISO-8859-1"), "UTF-8");
            String name = new String(fru.getName().getBytes("ISO-8859-1"), "UTF-8");
            fru.setTitle(Title);
            fru.setDescribes(Describes);
            fru.setName(name);
        ...
        return dao.UpdateFruit(fru);
    }

    @Override
    public Fruit FruitName(String name) {
        return dao.FruitName(name);
    }
    ...
    public int FruitImg(List<FileItem> items,String fruitname){
        ...
        if (!items.get(0).getName().equals("")) {
            ...
            //指定上传位置
            uploadFiilePath = "F:\\Shop_Background\\web\\imgs\\fruit\\" + name.substring(0, name.length() - 4);
            ...
            if (saveDir.listFiles().length+items.size() >= 5) {
                ...
                int b = dao.FruitImgByName(uploadFiilePath, fruitname);
                if (b == 0) {
                    return 0;
                }
            ...
            uploadFiilePath = "F:\\Shop_Background\\web\\imgs\\fruit\\" + UuidUtil.getUuid();
    ...
    @Override
    public int FruitUpdateImg(List<FileItem> items,String fruitname){
        Fruit fru=null;
        int leng=0;//长度
        int num=0;
        String []not_exists=new String[5];
        fru= dao.FruitName(fruitname);
        String uploadFiilePath=fru.getImgs();
        in_img imgs = new in_img();
        File saveDir = new File(uploadFiilePath);
        if (!items.get(0).getName().equals("")) {
            String name = items.get(0).getName();
    ...
    public int AddFruit(Fruit fruit) {
        SimpleDateFormat now_time = new SimpleDateFormat("yyyy-MM-dd");
        fruit.setAdd_time(now_time.format(new Date()));
        try {
            String Title = new String(fruit.getTitle().getBytes("ISO-8859-1"), "UTF-8");
            String Describes = new String(fruit.getDescribes().getBytes("ISO-8859-1"), "UTF-8");
            String name = new String(fruit.getName().getBytes("ISO-8859-1"), "UTF-8");
            fruit.setTitle(Title);
            fruit.setDescribes(Describes);
            fruit.setName(name);
